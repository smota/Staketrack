 ...
 *
 * Usage:
 * node scripts/update-firebase-config.js --env=dev|staging|prd [--no-confirm] [--dry-run] [--ci]
 */
...
if (!env || !['dev', 'staging', 'prd'].includes(env)) {
  console.error('Please specify a valid environment with --env=dev, --env=staging or --env=prd')
  process.exit(1)
}
